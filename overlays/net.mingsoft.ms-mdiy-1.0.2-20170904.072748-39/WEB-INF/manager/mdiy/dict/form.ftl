    ...
    <@ms.panel>
    	<@ms.form name="dictForm" isvalidation=true>
			...
			<#if dictEntity.dictType == "2">
    			...
    			<@ms.text label="分类" name="dictDescription" value="${dictEntity.dictDescription?default('')}"  width="240px;" placeholder="请输入描述" validation={"required":"false","maxlength":"50","data-bv-stringlength-message":"描述长度不能超过五十个字符长度!", "data-bv-notempty-message":"必填项目"}/>
