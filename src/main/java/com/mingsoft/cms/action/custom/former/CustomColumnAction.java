 ...
 */
@Controller
@RequestMapping("/${managerPath}/custom/column")
public class CustomColumnAction extends BaseAction{
	...
	public void weblist(@ModelAttribute ColumnEntity column,HttpServletResponse response, HttpServletRequest request,ModelMap model) {

		// 需要打开的栏目节点树的栏目ID
//		column.setColumnType(2);
//		categoryBiz.query
//		List<ColumnEntity> list = columnBiz.queryAll(1,7);
//		List<ColumnEntity> a = new ArrayList<>();
//		for (ColumnEntity columnEntity : list){
//			if(columnEntity.getColumnType() == 1){
//				a.add(columnEntity);
//			}
//		}
//		List list = columnBiz.query(column);
//		this.outJson(response, JSONArray.toJSONString(a));
		ColumnEntity column1 = (ColumnEntity) columnBiz.getEntity(column.getCategoryId());
		this.outJson(response, JSONArray.toJSONString(column1));
	}

	@SuppressWarnings("deprecation")
	@RequestMapping("/web/single")
	public void websingle(@ModelAttribute int columnId,HttpServletResponse response, HttpServletRequest request,ModelMap model) {
		ColumnEntity column = (ColumnEntity) columnBiz.getEntity(columnId);
		this.outJson(response, JSONArray.toJSONString(column));
	}
}
